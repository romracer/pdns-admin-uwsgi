...
: "${PDNS_ADMIN_SQLA_DB_HOST:='${PDNS_ENV_MYSQL_HOST:-mysql}'}"
: "${PDNS_ADMIN_SQLA_DB_PORT:='${PDNS_ENV_MYSQL_PORT:-3306}'}"
: "${PDNS_ADMIN_SQLA_DB_USER:='${PDNS_ENV_MYSQL_USER:-root}'}"
: "${PDNS_ADMIN_SQLA_DB_PASSWORD:='${PDNS_ENV_MYSQL_PASS:-powerdnsadmin}'}"
: "${PDNS_ADMIN_SQLA_DB_NAME:='${PDNS_ENV_MYSQL_DB:-powerdnsadmin}'}"
...
: "${PDNS_ADMIN_PDNS_STATS_URL:='http://${PDNS_ENV_PDNS_webserver_address:-pdns}:${PDNS_ENV_PDNS_webserver_port:-8081}/'}"
: "${PDNS_ADMIN_PDNS_API_KEY:='${PDNS_ENV_PDNS_api_key:-}'}"
: "${PDNS_ADMIN_PDNS_VERSION:='${PDNS_ENV_PDNS_version:-}'}"
